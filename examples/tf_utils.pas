...
uses
  ...
  tf_operations,                       // TGraph and the Generic ExecOper
...
procedure PrintTensorData(const ATensor:TF_TensorPtr; const AName:string='');
  ...
  case Length(GetTensorShape(ATensor)) of
    0:writeln(Format(AFormat,[Single(GetTensorValue(ATensor,[]))]));
    1:for i:=0 to GetTensorScalarCount(ATensor)-1 do
        writeln(Format(AFormat,[Single(GetTensorValue(ATensor,[i]))]));
    2:begin
      for i:=0 to GetTensorShape(ATensor)[0]-1 do
        begin
        for j:=0 to GetTensorShape(ATensor)[1]-1 do
          write(Format(AFormat,[Single(GetTensorValue(ATensor,[i,j]))]),chr(9));
        writeln;
        end;
      end
    else
      ...
      for i:=0 to GetTensorScalarCount(ATensor)-1 do
        ...
        for j:=0 to Length(Index)-1 do
          write(Index[j],' ');
        writeln('    ',Format(AFormat,[Single(GetTensorValue(ATensor,Index))]));
        end;
      end;
    end;
  end;
